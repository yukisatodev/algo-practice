...
**これは何か**：完成したプロダクトではなく、同じアルゴリズム問題を複数言語（Python / JavaScript / TypeScript / Java / C#）で解き比べる、日々の学習記録リポジトリです。

このプロジェクトから伝わればと思っている点です。

- **学習にも運用ルールを持たせている**：ファイル命名規則、言語間の対応付け、解法メモの記載といった方針を最初に言語化し（下記「2. 運用方針」参照）、思いつきではなく一貫した形で積み上げています。
- **「書けること」と「比較できること」を分けて考えている**：実務・独学で複数言語に触れてきた中で、「基本文法は書けるが横断的に比較した経験は少ない」という自分の課題を自覚した上で、それを埋めるために始めています。できることを並べるだけでなく、足りない部分を認識して対策する姿勢です。
- **詰まった問題も隠さず残している**：解けなかった問題も削除せず「未完了」として残す方針にしています（R-5）。進捗を実態より良く見せない、という誠実さの表れです。
- **Javaの制約にも実務的に対応している**：「ファイル名とpublicクラス名を一致させる必要がある」というJava特有の制約に対し、他言語と同じ命名規則を無理に当てはめず、フォルダ名で番号管理する形に調整しています。細かい言語仕様の違いを理解した上で運用を設計しています。
